...
const ItemCard = ({ item, user, setSelectedItem, setShowModal }) => (
  <motion.div
    layout
    className="bg-white/[0.02] border border-white/10 rounded-[2.5rem] p-6 hover:bg-white/[0.04] transition-all relative"
  >
    <div className="aspect-video bg-white/5 rounded-[1.5rem] mb-6 overflow-hidden flex items-center justify-center border border-white/5">
      {item.image_url ? (
        <img
          src={`https://astu-lost-find-system.onrender.com${item.image_url}`}
    ...
    <p className="text-cyan-400 text-[10px] font-bold uppercase mb-4 tracking-widest">
      {item.category}
    </p>
    ...
    {user.isAdmin && (
      <div className="mb-6 flex items-center gap-2 text-xs text-gray-400 bg-white/5 p-3 rounded-xl">
        <User size={14} />{" "}
        <span className="font-bold">By: {item.reporter_name}</span>
      </div>
    )}
